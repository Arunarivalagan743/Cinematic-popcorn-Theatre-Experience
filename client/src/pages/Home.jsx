...
//     setLoading(true);
//     try {
//       const response = await axios.get('https://mern-auth-movie-api.onrender.com');
...
import NewMovie from "../images/new.jpg";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faSearch, faTheaterMasks, faStar, faThumbsUp } from '@fortawesome/free-solid-svg-icons';
import { FaFilm, FaUser, FaNewspaper, FaTag } from 'react-icons/fa';
import Footer from '../components/Footer';
import { Link } from 'react-router-dom';
import { useSelector } from 'react-redux';
import withReactContent from 'sweetalert2-react-content';
  ...
  const fetchMovies = async () => {
    ...
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        setMovies([]); // Default to empty array if response is not an array
  ...
  return (
    <div className="min-h-screen p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredMovies.map((movie) => (
          <div key={movie._id} onClick={() => showMovieDetails(movie)} className="cursor-pointer">
            <img
              src={imageMap[movie.imageUrl] || 'default.jpg'}
